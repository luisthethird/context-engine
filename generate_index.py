...
def extract_frontmatter(path: Path) -> tuple[list[str], str]:
    ...
    try:
        ...
        if m:
            ...
            if block:
                tags = [re.sub(r"^\s*-\s*", "", ln).strip() for ln in block[0].splitlines() if ln.strip()]
...
def file_node(path: Path, rel: Path, override: dict = None) -> dict:
    ...
    if not tags:
        tags = infer_tags(rel)
    if override:
        tags = sorted(set(tags) | set(override.get("tags", [])))
...
def dir_node(dir_path: Path, vault_root: Path) -> dict:
    ...
    for ext, files in file_groups.items():
        if len(files) >= COLLECTION_THRESHOLD:
            children.append({
                ...
                "tags": infer_tags(rel / files[0].name),
                "summary": f"{dir_path.name} ({ext} files)",
    ...
    all_tags = set(infer_tags(rel, is_dir=True)) | set(sidecar.get("tags", []))
    for c in children:
...
def build_manifest(repos: list[dict]) -> dict:
    ...
    for r in repos:
        name = r["name"]
        slice_file = f"claude-index-{name}.json"
...
def main():
    ...
    write(out / "claude-index.json", {"repos": nodes}, args.dry_run)

    if args.split:
        index_dir = out / "index"
        for node in nodes:
            write(
                index_dir / f"claude-index-{node['name']}.json",
                {"meta": {"repo": node["name"], "split_version": "1",
                           "usage": "Per-repo slice. Load after manifest lookup."},
                 "repo": node},
                args.dry_run,
            )
        write(index_dir / "claude-index-manifest.json",
