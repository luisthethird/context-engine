#!/usr/bin/env python3
"""
generate_index_webdev.py - LLM context index generator (web development preset)

A web-development-focused variant of generate_index.py with tag maps tuned for
static sites, template-based builds (Jinja2, Handlebars, etc.), Tailwind CSS,
and typical frontend asset structures.

Usage:
    python3 generate_index_webdev.py --vault /path/to/workspace --output /path/to/workspace --split

Customization:
    Edit PATH_TAG_MAP, FILENAME_TAG_MAP, and EXT_TAG_MAP below to match your
    project conventions. Use _meta.json sidecars in any directory for explicit
    overrides (see README.md).
"""
...
# ── Web development tag maps ─────────────────────────────────────────────
# Map path component keywords to domain tags.
PATH_TAG_MAP = [
    ("assets",      ["assets", "media", "images"]),
    ("templates",   ["template", "frontend"]),
    ("components",  ["components", "frontend", "ui"]),
    ("styles",      ["css", "styling", "frontend"]),
    ("scripts",     ["scripts", "tooling"]),
    ("pages",       ["pages", "frontend", "html"]),
    ("layouts",     ["layouts", "template", "frontend"]),
    ("partials",    ["partials", "template", "frontend"]),
...
FILENAME_TAG_MAP = [
    ("template",        ["template", "frontend", "html"]),
    ("config",          ["config", "content"]),
    ("README",          ["docs"]),
    ("CHANGELOG",       ["docs", "history"]),
    ("index.html",      ["build-output", "html"]),
    ("favicon",         ["branding", "assets"]),
    ("logo",            ["branding", "assets"]),
    ("manifest",        ["config", "pwa"]),
    ("tailwind",        ["tailwind", "css", "styling"]),
    ("postcss",         ["css", "styling", "build"]),
...
EXT_TAG_MAP = {
    ...
    ".less":  ["css", "frontend", "styling"],
    ".js":    ["javascript", "frontend"],
    ".jsx":   ["javascript", "frontend", "react"],
    ".ts":    ["typescript", "frontend"],
    ".tsx":   ["typescript", "frontend", "react"],
    ".vue":   ["vue", "frontend"],
...
SKIP_DIRS = {
    ...
    ".nuxt", ".svelte-kit", ".github", ".vercel", ".netlify",
...
SKIP_EXTS = {".pyc", ".pyo", ".pkl", ".pickle", ".swp", ".swo", ".map"}
...
def main():
    parser = argparse.ArgumentParser(description="Generate LLM context index (web-dev preset)")
