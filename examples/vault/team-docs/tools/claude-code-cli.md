...
description: Running and scripting Claude Code CLI against this workspace (interactive, print mode, isolation)
---
...
```bash
# Find your session ID in ~/.claude/projects/ or from the session list
...
```
[context-engine] manifest loaded: 3 repos, N tags
[context-engine] repos: team-data, team-docs, team-infra
```

(N = tag count for your workspace; the example workspace has 29 after the tool guides are indexed.)
...
3. `examples/vault/index/manifest.json` exists (run `bash examples/setup.sh` if not)
